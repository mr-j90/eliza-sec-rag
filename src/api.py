...
so the frontend can label the UI from the backend rather than keeping a second, driftable copy
(D001).
...
class AskRequest(BaseModel):
    ...
    def not_only_whitespace(cls, value: str) -> str:
        ...
        provider and spend a generation call on a non-question. Filed as a defect against
        I001 and fixed here (G00 route entry 1)."""
...
def provider_not_configured(_request: Any, exc: ProviderNotConfigured):
    """503 with the reason, never a plausible-looking answer (I001 signal 4)."""
